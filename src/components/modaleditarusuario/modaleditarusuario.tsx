...
const {rotaeditarusuario} = usarcontextoapi();
...
try {
  setLoading(true)
  const response = await fetch(rotaeditarusuario, {
  method: 'POST',
  headers: {
...
useEffect(()=>{
  ...
  return (
    ...
    <form className='form-modal-usuario' onSubmit={atualizar_usuario}>
      ...
      <div className='container-input'>
        <span className='span-modal-usuario'>Tipo:</span>
        <div className='container-input-icon-modal'>
         <select className='input-modal-usuario'  ref={tipo} disabled={!editando.tipo}  defaultValue={Selectionmodal?.tipo}  >
...
export default Modal_editar_usuario
